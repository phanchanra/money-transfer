...
let indexTmpl = Template.MoneyTransfer_customer,
    ...
    newTmpl = Template.MoneyTransfer_customerNew,
    editTmpl = Template.MoneyTransfer_customerEdit,
...
indexTmpl.helpers({
    ...
    tableSettings(){
        let i18nPrefix = 'moneyTransfer.customer.schema';

        reactiveTableSettings.collection = 'moneyTransfer.reactiveTable.customer';
        reactiveTableSettings.filters = ['moneyTransfer.customerByBranchFilter'];
...
indexTmpl.events({
    'click .js-create' (event, instance) {
        alertify.customer(fa('plus', TAPi18n.__('moneyTransfer.customer.title')), renderTemplate(newTmpl));
    },
    'click .js-update' (event, instance) {
        alertify.customer(fa('pencil', TAPi18n.__('moneyTransfer.customer.title')), renderTemplate(editTmpl, this));
    },
    ...
    'click .js-display' (event, instance) {
        alertify.customerShow(fa('eye', TAPi18n.__('moneyTransfer.customer.title')), renderTemplate(showTmpl, this));
    }
});
...
showTmpl.helpers({
    i18nLabel(label){
        let i18nLabel = `moneyTransfer.customer.schema.${label}.label`;
        return i18nLabel;
    },
    data () {
        let data = Customer.findOne(this._id);
...
AutoForm.addHooks([
    'MoneyTransfer_customerNew',
    'MoneyTransfer_customerEdit'
], hooksObject);
